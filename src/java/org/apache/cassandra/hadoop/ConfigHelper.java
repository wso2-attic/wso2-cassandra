...
public class ConfigHelper
{
    ...
    {
        String trustStore = conf.get("ssl.truststore", null);
        String trustStorePassword = conf.get("ssl.truststore.password", null);

        String factoryClassName;

        // This is a temporary fix for Cassandra SSL issue with hive/hadoop. We are switching secure and non-secure socket based on
        ...
        if ((trustStore != null && !trustStore.isEmpty()) && (trustStorePassword != null && !trustStorePassword.isEmpty())) {
            factoryClassName = SecureTFramedTransportFactory.class.getName();
        } else {

            factoryClassName = conf.get(
